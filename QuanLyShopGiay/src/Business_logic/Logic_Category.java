 ...
 * @author lengo
 */
public class Logic_Category {

    private Category ui;
    private JPanel uiContain;
    private Logic_Admin_Home lgHome;
    private SQL_DanhMuc data;

    public Logic_Category(Category ui) {
        this.ui = ui;
    }
    ...
    public void loadDefault() {
        List<ENTITY.Entity_DanhMuc> list = data.instance().instance().getLists();
        initTable(list);
    }
    ...
    public void initTable(List<Entity_DanhMuc> list) {
        ...
        for (ENTITY.Entity_DanhMuc x : list) {
    ...
    public void loadAdd() {
        Entity_DanhMuc item = getItemSelect();
    ...
    public void loadSearch(String valueSearch) {
        List<Entity_DanhMuc> list = data.instance().getListsByName(valueSearch);
        if (list == null) {
            loadDefault();
        } else {
            initTable(list);
    ...
    public void loadInfo(){
        ...
        Entity_DanhMuc item = new Entity_DanhMuc((int)ui.table.getValueAt(row, 0), (String) ui.table.getValueAt(row, 1));
    ...
    public Entity_DanhMuc getItemSelect() {
        return new Entity_DanhMuc(Integer.parseInt(ui.ID.getText()), ui.name.getText());
    }
}
